...
function setupEventListeners() {
  ...
  companyGroupSelector.addEventListener('change', (e) => {
    ...
    if (companyGroupId) {
      dashboardLoadedForGroupId = null; // グループ変更時はキャッシュ破棄
...
function handleFileSelect(event) {
  ...
  reader.onload = async function (e) {
    try {
      ...
      dashboardLoadedForGroupId = null; // ダッシュボードも再取得させる
...
// --- モジュールレベル変数 ---
let dashboardCastsData = []; // { castId, castName, todayRevenue, yesterdayRevenue, thirtyDayStats }[]
let currentTab = 'dashboard';
let overallRevenueChartInstance = null;
let overallAccessChartInstance = null;
// ダッシュボードキャッシュ: 同じグループで再タップした場合は再取得しない
...
window.refreshDashboard = function() {
  dashboardLoadedForGroupId = null; // キャッシュ強制クリア
  ordersRangeCache.clear();
  noDataCastIds.clear();
...
// ダッシュボード: データ読み込み
// -------------------------------------------------------------------
async function loadDashboardData(companyGroupId) {
  if (!companyGroupId) return;

  const loadingEl = document.getElementById('dashboardLoading');
  const cardsEl = document.getElementById('castCardsContainer');
  ...
  const yesterdayStr = toDateStr(yesterdayStart);
  const thirtyDaysAgo = new Date(now.getTime() - 30 * 24 * 60 * 60 * 1000);

  // キャスト一覧を castSelector から取得
  const casts = [];
  if (castSelector) {
  ...
  // プログレスバーを初期化
  let dashCompleted = 0;
  const dashTotal = casts.length;
  const updateDashProgress = (n) => {
    const pct = Math.round((n / dashTotal) * 100);
    const bar = document.getElementById('dashboardProgressBar');
    const pctEl = document.getElementById('dashboardProgressPercent');
    const textEl = document.getElementById('dashboardProgressText');
    if (bar) bar.style.width = pct + '%';
    if (pctEl) pctEl.textContent = pct + '%';
    if (textEl) textEl.textContent = `${n} / ${dashTotal} キャスト読み込み中...`;
  };
  updateDashProgress(0);

  try {
    // 全キャストのデータを並列取得
    // 30日分を1回だけ取得して、今日/昨日の売上もそこから算出する（クエリ数を1/3に圧縮）
    const results = await Promise.all(casts.map(async (cast) => {
      const thirtyRecs = await fetchOrdersByPeriod(cast.id, companyGroupId, thirtyDaysAgo, now);
      updateDashProgress(++dashCompleted);

      // 30日間の日別売上
      const dailyMap = {};
      thirtyRecs.forEach(r => {
        if (r.status !== '取引完了') return;
        const d = (r.orderDate || '').split(' ')[0].replace(/\//g, '-').slice(0, 10);
        if (!d) return;
        dailyMap[d] = (dailyMap[d] || 0) + (r.price || 0);
      });

      const todayRevenue = dailyMap[todayStr] || 0;
      const yesterdayRevenue = dailyMap[yesterdayStr] || 0;

      return { castId: cast.id, castName: cast.name, todayRevenue, yesterdayRevenue, dailyMap };
    }));

    dashboardCastsData = results;
    dashboardLoadedForGroupId = companyGroupId; // キャッシュ確定
  } catch (err) {
...
function renderDashboard() {
  renderDashboardDateLabel();
  renderDashboardAlerts();
  renderCastCards();
  renderOverallRevenueChart();
  renderDashboardAccessChart();
}

// -------------------------------------------------------------------
// ダッシュボード: 日付ラベル
// -------------------------------------------------------------------
function renderDashboardDateLabel() {
  ...
  el.textContent = `${y}年${m}月${d}日 (${weekdays[now.getDay()]}) のデータ`;
}
...
function renderDashboardAlerts() {
  ...
  // キャスト別売上アラート (30%以上の変動)
  dashboardCastsData.forEach(cast => {
    if (cast.yesterdayRevenue <= 0) return;
    const change = calcChangePercent(cast.todayRevenue, cast.yesterdayRevenue);
    if (Math.abs(change) >= 30) {
      const isUp = change > 0;
      alerts.push({
        type: isUp ? 'up' : 'down',
        message: `${cast.castName}: 昨日比 ${isUp ? '+' : ''}${change}%（昨日 ${cast.yesterdayRevenue.toLocaleString()}円 → 今日 ${cast.todayRevenue.toLocaleString()}円）`,
      });
    }
  });

  // アクセスアラート (globalAccessDataMap から today/yesterday)
  if (globalAccessDataMap && globalAccessDataMap.size > 0) {
    const now = new Date();
    const todayStr = toDateStr(now);
    const yesterdayDate = new Date(now.getTime() - 24 * 60 * 60 * 1000);
    const yesterdayStr = toDateStr(yesterdayDate);
    const todayAccess = globalAccessDataMap.get(todayStr) || 0;
    const yesterdayAccess = globalAccessDataMap.get(yesterdayStr) || 0;
    if (yesterdayAccess > 0) {
      const change = calcChangePercent(todayAccess, yesterdayAccess);
      if (Math.abs(change) >= 30) {
        const isUp = change > 0;
        alerts.push({
          type: isUp ? 'up' : 'down',
          message: `アクセス数: 昨日比 ${isUp ? '+' : ''}${change}%（昨日 ${yesterdayAccess.toLocaleString()} → 今日 ${todayAccess.toLocaleString()}）`,
        });
      }
    }
  }
  ...
  alerts.forEach(alert => {
    ...
    div.innerHTML = `
      <span class="text-base">${isUp ? '📈' : '📉'}</span>
...
function renderCastCards() {
  const container = document.getElementById('castCardsContainer');
  if (!container) return;
  container.innerHTML = '';

  if (dashboardCastsData.length === 0) {
    container.innerHTML = '<p class="col-span-full text-center text-slate-400 text-sm py-8">キャストデータがありません。キャストを追加してください。</p>';
    return;
  }
  ...
  dashboardCastsData.forEach(cast => {
    const change = calcChangePercent(cast.todayRevenue, cast.yesterdayRevenue);
    const isUp = change >= 0;
    const hasYesterday = cast.yesterdayRevenue > 0;

    const card = document.createElement('div');
    card.className = 'bg-white rounded-xl border border-slate-200 shadow-sm p-4 cursor-pointer hover:border-blue-400 hover:shadow-md transition-all';
    card.onclick = () => window.switchToCastDetail(cast.castId);

    const changeBadge = hasYesterday
      ? `<span class="inline-flex items-center gap-0.5 text-xs font-semibold px-1.5 py-0.5 rounded-full ${
          isUp ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'
        }">${isUp ? '▲' : '▼'} ${Math.abs(change)}%</span>`
      : '<span class="text-xs text-slate-400">-</span>';

    card.innerHTML = `
      <div class="flex items-start justify-between mb-2">
        <p class="text-xs font-medium text-slate-500 truncate max-w-[70%]">${cast.castName}</p>
        ${changeBadge}
      </div>
      <p class="text-xl font-bold text-slate-800">${cast.todayRevenue.toLocaleString()}<span class="text-sm font-normal text-slate-400 ml-1">円</span></p>
      <p class="text-xs text-slate-400 mt-1">昨日: ${cast.yesterdayRevenue.toLocaleString()}円</p>
    `;
...
// -------------------------------------------------------------------
// ダッシュボード: 全体売上チャート (30日)
// -------------------------------------------------------------------
function renderOverallRevenueChart() {
  const canvas = document.getElementById('overallRevenueChart');
  if (!canvas) return;

  if (overallRevenueChartInstance) {
    overallRevenueChartInstance.destroy();
    overallRevenueChartInstance = null;
  }
  ...
  dashboardCastsData.forEach(cast => {
    days.forEach(d => {
      combined[d] += (cast.dailyMap[d] || 0);
    });
  });

  const labels = days.map(d => d.slice(5)); // MM-DD
  const data = days.map(d => combined[d]);

  overallRevenueChartInstance = new Chart(canvas, {
    type: 'bar',
    data: {
      labels,
      datasets: [{
...
// -------------------------------------------------------------------
// ダッシュボード: アクセスチャート (globalAccessDataMap 使用)
// -------------------------------------------------------------------
function renderDashboardAccessChart() {
  const canvas = document.getElementById('overallAccessChart');
  const note = document.getElementById('overallAccessChartNote');
  if (!canvas) return;

  if (overallAccessChartInstance) {
    overallAccessChartInstance.destroy();
    overallAccessChartInstance = null;
  }
  ...
  const hasData = globalAccessDataMap && days.some(d => globalAccessDataMap.has(d));
  if (note) note.classList.toggle('hidden', hasData);

  const data = days.map(d => (globalAccessDataMap ? globalAccessDataMap.get(d) ?? null : null));
  const labels = days.map(d => d.slice(5));

  overallAccessChartInstance = new Chart(canvas, {
    type: 'line',
    data: {
      labels,
      datasets: [{
    ...
    options: {
      ...
      plugins: {
        ...
        tooltip: {
          callbacks: {
            label: ctx => ctx.parsed.y !== null ? `${ctx.parsed.y.toLocaleString()}` : 'データなし',
          },
        },
      },
      scales: {

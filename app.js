...
/**
 * ordersの配列を月次アーカイブとしてFirestoreに保存します（チャンク分割あり）。
 * chunkId: orders <= CHUNK_SIZE なら yearMonth のみ、超える場合は yearMonth_1, yearMonth_2 ...
 */
async function saveArchiveChunks(monthlyColRef, yearMonth, orders) {
  const ts = new Date().toISOString();
  const saveBatch = writeBatch(db);
  if (orders.length === 0) {
    saveBatch.set(doc(monthlyColRef, yearMonth), { orders: [], lastUpdated: ts });
  } else {
    for (let i = 0; i * ARCHIVE_CHUNK_SIZE < orders.length; i++) {
      const chunk = orders.slice(i * ARCHIVE_CHUNK_SIZE, (i + 1) * ARCHIVE_CHUNK_SIZE);
      const chunkId = i === 0 ? yearMonth : `${yearMonth}_${i}`;
      saveBatch.set(doc(monthlyColRef, chunkId), { orders: chunk, lastUpdated: ts });
    }
  }
...
async function buildAndSaveMonthlyArchive(castId, companyGroupId, yearMonth) {
  ...
  await saveArchiveChunks(monthlyColRef, yearMonth, orders);
...
async function fetchAllOrdersViaArchives(castId, companyGroupId) {
  ...
  if (archiveSnap.empty) {
    ...
    let allSnap;
    try {
      allSnap = await getDocs(query(ordersColRef));
    } catch (e) {
      console.warn('全件取得に失敗しました（Quota等）:', e.message);
      allTimeOrdersCache.set(castKey, []);
      return [];
    }
    const ordersByMonth = {};
    allRecords = [];
    allSnap.forEach(d => {
      const data = d.data();
      const normalizedDate = (data.orderDate || '').replace(/\//g, '-');
      const month = normalizedDate.slice(0, 7);
      if (month && /^\d{4}-\d{2}$/.test(month)) {
        if (!ordersByMonth[month]) ordersByMonth[month] = [];
        ordersByMonth[month].push({ ...data, orderDate: normalizedDate });
      }
      allRecords.push({ ...data, orderDate: normalizedDate });
    });
    // アーカイブをバックグラウンドで保存（チャンク分割）
    for (const [month, orders] of Object.entries(ordersByMonth)) {
      saveArchiveChunks(monthlyColRef, month, orders)
        .catch(e => console.warn(`アーカイブ作成失敗 ${month}:`, e.message));
    }
  } else {
    allRecords = [];
    archiveSnap.forEach(d => allRecords.push(...(d.data().orders || [])));
    console.log(`月次アーカイブから読み込み: ${archiveSnap.size}チャンク, ${allRecords.length}件`);
  }
...
async function fetchOrdersByPeriod(castId, companyGroupId, startDate, endDate) {
  ...
  if (allTimeOrdersCache.has(castKey)) {
    const records = allTimeOrdersCache.get(castKey).filter(r => {
      const dateStr = (r.orderDate || '').replace(/\//g, '-').slice(0, 10);
...
async function loadAccessDataForCast(castName) {
  ...
  try {
    ...
    snap.forEach(docSnap => {
      ...
      const channelEntry = {};
      for (const ch of ['Direct', 'Organic Social', 'Referral', 'Organic Search', 'Email']) {
        if (typeof data[ch] === 'number') channelEntry[ch] = data[ch];
      }
...
function renderProductTypeTabs() {
  ...
  planTab.textContent = 'プラン状況';
  planTab.dataset.type = 'plan';
  planTab.className = 'px-4 py-2 rounded-lg font-medium text-sm transition-colors ' +
    (currentProductTypeFilter === 'plan' ? 'bg-purple-600 text-white shadow-md' : 'bg-white text-purple-700 hover:bg-purple-50 border border-purple-300');
  planTab.onclick = () => handleProductTypeChange('plan');
  productTypeTabs.appendChild(planTab);
}
...
function updateViewFromGlobalData() {
  ...
  // 結果表示
  displayResults(dailyStats, productStats, totalRevenue, totalQuantity, fanclubMrr);
}
 ...
 */
function displayResults(dailyStats, productStats, totalRevenue, totalQuantity, fanclubMrr = 0) {
  // ジャンル表示用のラベル作成
  ...
  const yRev = dailyStats[yesterday]?.revenue || 0;
  const dbRev = dailyStats[dayBefore]?.revenue || 0;
  let yesterdayBannerHtml = '';
  if (!(yRev === 0 && dbRev === 0)) {
    const pct = dbRev > 0 ? ((yRev - dbRev) / dbRev) * 100 : null;
    const pctText = pct === null ? '' : ` <span class="text-xs">(${pct >= 0 ? '+' : ''}${pct.toFixed(1)}%)</span>`;
    if (pct !== null && pct >= 30) {
      yesterdayBannerHtml = `
        <div class="rounded-xl px-4 py-3 mb-4 flex items-center gap-3 bg-green-50 border border-green-200">
          <span class="text-2xl">📈</span>
          <div>
            <p class="text-sm font-semibold text-green-800">昨日の売上: ¥${yRev.toLocaleString()}${pctText}</p>
            <p class="text-xs text-green-700">前日比30%以上増加！</p>
          </div>
        </div>
      `;
    } else if (pct !== null && pct <= -30) {
      yesterdayBannerHtml = `
        <div class="rounded-xl px-4 py-3 mb-4 flex items-center gap-3 bg-red-50 border border-red-200">
          <span class="text-2xl">📉</span>
          <div>
            <p class="text-sm font-semibold text-red-800">昨日の売上: ¥${yRev.toLocaleString()}${pctText}</p>
            <p class="text-xs text-red-700">前日比30%以上減少</p>
          </div>
        </div>
      `;
    } else {
      yesterdayBannerHtml = `
        <div class="rounded-xl px-4 py-3 mb-4 flex items-center gap-3 bg-slate-50 border border-slate-200">
          <span class="text-2xl">💹</span>
          <div>
            <p class="text-sm font-semibold text-slate-800">昨日の売上: ¥${yRev.toLocaleString()}${pctText}</p>
            <p class="text-xs text-slate-600">前日比の変動は30%未満です</p>
          </div>
        </div>
      `;
    }
  ...
  resultsContainer.innerHTML = `
            ${yesterdayBannerHtml}
            ${createSummaryCard(totalRevenue, totalQuantity, productStats, genreLabel, fanclubMrr)}

            <div class="bg-white rounded-xl shadow-lg p-6 border border-gray-200 mb-4">
                <div class="flex items-center justify-between mb-2 gap-2 flex-wrap">
                    <h2 class="text-lg font-bold text-gray-800">売上推移・CVR・アクセスチャンネル</h2>
                    <div id="channelToggleArea" class="flex flex-wrap gap-3 text-xs"></div>
                </div>
            ...
            <div class="bg-white rounded-xl shadow-lg p-6 border border-gray-200 mt-6">
                <h2 class="text-lg font-bold text-gray-800 mb-4">ジャンル別売上比率${genreLabel}</h2>
...
function renderCharts(dailyStats) {
    ...
    if (toggleArea) {
      ...
      CHANNEL_CONFIG.forEach((ch, i) => {
        ...
        label.innerHTML = `
          <input type="checkbox" checked class="rounded">
          <span style="color:${ch.color}" class="font-medium">${ch.label}</span>
        `;
        label.querySelector('input').addEventListener('change', (e) => {
          salesChartInstance.setDatasetVisibility(datasetIndex, e.target.checked);
          salesChartInstance.update();
        });
...
function createSummaryCard(totalRevenue, totalQuantity, productStats, genreLabel, fanclubMrr = 0) {
  ...
  const fanclubRow = fanclubMrr > 0 ? `
    <div class="mt-3 pt-3 border-t border-gray-100 flex flex-wrap justify-center gap-x-6 gap-y-1 text-sm text-center">
      <span class="text-gray-500">注文売上 <span class="font-semibold text-gray-700">¥${totalRevenue.toLocaleString()}</span></span>
      <span class="text-gray-400">＋</span>
      <span class="text-gray-500">ファンクラブMRR <span class="font-semibold text-purple-600">¥${fanclubMrr.toLocaleString()}</span></span>
      <span class="text-gray-400">=</span>
      <span class="text-gray-500">合計 <span class="font-bold text-blue-700">¥${(totalRevenue + fanclubMrr).toLocaleString()}</span></span>
    </div>
  ` : '';
  ...
  return `
        <div class="bg-white rounded-xl shadow-lg p-6 border border-gray-200">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 text-center">
                <div>
                    <p class="text-sm text-gray-500">総売上金額${fanclubMrr > 0 ? '（注文）' : ''}</p>
...
async function loadDashboardData(groupId) {
  ...
  const pct = document.getElementById('dashboardProgressPercent');
  const text = document.getElementById('dashboardProgressText');
  const cards = document.getElementById('castCardsContainer');
